...
DEFAULT_SERVICE_NAME="google-cloud-e2e"
...
dart compile exe bin/server.dart \
  -o ${BUILD_DIR}/${BINARY_LOCATION} \
  --target-arch x64 \
  --target-os linux \
  >&2
# No public/ directory to copy
...
gcloud beta run deploy ${SERVICE_NAME} \
  ...
  --format="json(status.address.url, status.latestReadyRevisionName)"
